...
async function digestTitles(owner: Device): Promise<string[]> {
...
describe('history retention', () => {
  it('refreshes last-observed time for every Feed Item the Feed Window still exposes', async () => {
    ...
    await subscribed(owner, service, rss(item('a', 'Kept'), item('b', 'Dropped')))
  ...
  it('prunes an unsaved Feed Item 90 days after its last observation, to the minute', async () => {
    ...
    // Three months idled the session out; the Owner signs back in to look.
    expect((await owner.signIn()).status).toBe(200)
    expect(await digestTitles(owner)).toEqual(['Kept'])
  })
  ...
  it('retains an item a slow Feed still exposes long after its publication date', async () => {
    ...
    await subscribed(owner, service, rss(item('a', 'Yearly letter', '2026-01-05T08:00:00.000Z')))
    ...
    expect((await owner.signIn()).status).toBe(200)
    expect(await digestTitles(owner)).toEqual(['Yearly letter'])
  })

  it('never prunes the Library, however old and whatever happened to the Feed', async () => {
    const service = await startTestService()
    const owner = await claimedDevice(service)
    await subscribed(owner, service, rss(item('a', 'Saved essay'), item('b', 'Passing note')))
    const saved = await owner.put('/api/library/1')
    ...
    await service.wakeScheduler()
    expect(storedItems(service).map((entry) => entry.title)).toEqual(['Saved essay'])
    expect((await library(owner)).items.map((entry) => [entry.title, entry.feedTitle, entry.subscribed])).toEqual([
  ...
  it('keeps an aged item that the same wake re-observes — the poll counts before the sweep', async () => {
    const service = await startTestService()
    const owner = await claimedDevice(service)
    await subscribed(owner, service, rss(item('a', 'Quarterly letter')))
  ...
  it('unsubscribing stops polling immediately and clears the Digest of that Feed', async () => {
    const service = await startTestService()
    const owner = await claimedDevice(service)
    await subscribed(owner, service, rss(item('a', 'First light')))
    expect(await digestTitles(owner)).toEqual(['First light'])

    expect((await owner.delete('/api/feeds/1')).status).toBe(204)

    // Gone from the Digest and the Subscription list at once, not at cleanup.
    expect(await digestTitles(owner)).toEqual([])
    const list = subscriptionListSchema.parse(await (await owner.get('/api/feeds')).json())
    expect(list.subscriptions).toEqual([])
    expect((await owner.get('/api/feeds/1')).status).toBe(404)
    ...
    expect((await owner.delete('/api/feeds/1')).status).toBe(404)
    expect((await owner.delete('/api/feeds/abc')).status).toBe(404)
  })
  ...
  it('cleanup clears an unsubscribed Feed but keeps saves and their attribution', async () => {
    ...
    expect((await owner.delete('/api/library/1')).status).toBe(200)
    await service.wakeScheduler()
    expect(storedItems(service)).toEqual([])
  ...
  it('sweeps a bounded batch per wake and converges over the following wakes', async () => {
    ...
    const owner = await claimedDevice(service)
    await subscribed(owner, service, rss(item('a', 'One'), item('b', 'Two'), item('c', 'Three')))
  ...
  it('cannot be raced by an in-flight poll into resurrecting an unsubscribed Feed', async () => {
    ...
    // A manual refresh is mid-retrieval when the Owner unsubscribes.
    ...
    const refreshing = owner.post('/api/feeds/1/refresh')
  ...
  it('revives a Feed retained for the Library when the Owner resubscribes', async () => {
    const service = await startTestService()
    const owner = await claimedDevice(service)
    await subscribed(owner, service, rss(item('a', 'Saved essay'), item('b', 'Passing note')))
    expect((await owner.put('/api/library/1')).status).toBe(200)
    ...
    const revived = await owner.post('/api/subscriptions', { url: FEED_URL })
    ...
    expect((await digestTitles(owner)).sort()).toEqual(['Passing note', 'Saved essay'])
    expect((await library(owner)).items.map((entry) => [entry.feedItemId, entry.title])).toEqual([
      [1, 'Saved essay'],
    ])
