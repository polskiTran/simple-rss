...
describe('migrations', () => {
  ...
  it('rolls the whole migration back when its statements fail', async () => {
    ...
    const table = db.$client.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'half_done'").get()
  ...
  it('makes a second User unrepresentable rather than merely refused', async () => {
    ...
    const insert = db.$client.prepare(
      'INSERT INTO user_auth (id, password_hash, claimed_at, updated_at) VALUES (?, ?, ?, ?)',
    )
  ...
  it('leaves a Feed recorded before the home page column ready to fill it in on the next poll', async () => {
    ...
    db.$client
      .prepare(`INSERT INTO feeds (entered_url, resolved_url, title, domain, etag, last_modified, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(
    ...
    expect(applied).toEqual([8, 9, 10, 11, 12, 13])
  ...
  it('carries an already-claimed volume through the rename to user_auth', async () => {
    ...
    db.$client
      .prepare('INSERT INTO owner_auth (id, password_hash, claimed_at, updated_at) VALUES (?, ?, ?, ?)')
      .run(1, '$argon2id$claimed-before-the-rename', '2026-01-01T00:00:00.000Z', '2026-01-01T00:00:00.000Z')
    ...
    const row = db.$client.prepare('SELECT password_hash, claimed_at FROM user_auth').get() as {
