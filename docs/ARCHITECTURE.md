...
Subscribing proves the Feed inside the request (ADR 0009). `POST /api/feeds/preview` retrieves and parses the pasted address and records nothing, so the dialog can ask `subscribe to <title>?` before anything is written. `POST /api/subscriptions` then retrieves and parses the Feed URL the preview settled on, unconditionally, and writes the Feed, the Subscription, and the first Feed Window in one transaction. It answers 201 with a Subscription already available, its items already in the Digest and its next poll one Polling Interval out. Subscribe never wakes the scheduler. The request settles duplicates after the proof and before the write. The Feed owning the resolved URL, else the typed URL, is the Feed. The request answers 409 when that Feed is subscribed, revives a dormant Feed into the same row, and creates a Feed when neither URL has an owner. When the address does not prove to be a Feed, the request answers 4xx with the reason and leaves no row. A page that reaches subscribe is refused with `no_feed_found`, because reading declarations belongs to the preview.

OPML Import records each entry without contacting its Feed (ADR 0009). The server validates the URL shape, deduplicates against known Feed URLs, and creates the Feed and Subscription transactionally; the Subscription starts unchecked and immediately due, and the import nudges the scheduler awake. The first retrieval is an ordinary poll: it confirms RSS or Atom content, corrects the Feed's title and resolved URL, and ingests the current Feed Window. When a poll resolves onto a URL another Feed already owns, the later Subscription merges quietly into that Feed. OPML Import is what makes that ordinary, since it alone records a URL before seeing where it resolves. A subscribed Feed whose publisher later redirects onto another folds in the same way.

The entered URL is preserved while a validated resolved URL may be recorded for subsequent retrievals. One OPML Import records at most 500 Feeds. A wake that finds a full due batch drains the next batch at once, and a Feed the retrieval budget turned away waits one wake interval instead of a full Polling Interval, so an import's rows fill in at the pace of the retrieval budgets rather than one batch per minute.
...
Retrieval gives a pasted address a profile of its own, `preview` (ADR 0005). The profile keeps the Feed ceiling, the redirect count, and the destination rules, and it accepts a page as well as a Feed. One fifteen-second deadline covers the answer and its body together, and the page and the Declared Feed it leads to share that one deadline. The profile's concurrency budget is separate from polling's, so a running OPML Import can neither refuse nor delay a preview.
