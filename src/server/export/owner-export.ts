...
export const OWNER_EXPORT_FORMAT = 'simple-rss-export'
 ...
 */
export const OWNER_EXPORT_VERSION = 1
 ...
 */
export interface OwnerExportItem {
...
/**
 * One Feed with the Owner's relationship to it. `subscription` is null for a
 * Feed kept only because Library saves still attribute to it.
 */
export interface OwnerExportFeed {
  ...
  readonly items: readonly OwnerExportItem[]
}

/**
 * The Owner's portable reading state, complete enough to carry Subscriptions,
 ...
 * Those are this installation's operational property, not the Owner's reading.
 */
export interface OwnerExport {
  readonly format: typeof OWNER_EXPORT_FORMAT
  readonly exportVersion: typeof OWNER_EXPORT_VERSION
  ...
  readonly feeds: readonly OwnerExportFeed[]
}
...
interface ItemRow extends Omit<OwnerExportItem, 'identityKind'> {
  identityKind: string
}
 ...
 */
export function buildOwnerExport(options: {
  ...
  return database.transaction((): OwnerExport => {
    ...
    const feeds = feedRows.map(
      (feed): OwnerExportFeed => ({
        ...
        items: (itemsOfFeed.all(feed.id) as ItemRow[]).map(
          (item): OwnerExportItem => ({ ...item, identityKind: item.identityKind as OwnerExportItem['identityKind'] }),
        ),
      }),
    )

    return {
      format: OWNER_EXPORT_FORMAT,
      exportVersion: OWNER_EXPORT_VERSION,
